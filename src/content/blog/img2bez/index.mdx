...
**2. Adaptive smoothing.** The boundary is resampled at 1px arc-length spacing and smoothed with a small Gaussian ($\sigma = 1.2$ samples). If the result still doesn't read as clean type geometry—mean absolute turn above $0.10$ radians per sample, or corners detected more often than once per 30 samples, which real glyphs never do—$\sigma$ escalates by $1.8\times$ and tries again, up to four times. Clean renders never escalate; degraded sources get progressively stronger treatment at their own noise scale.

**3. Structure detection from curvature.** Every structural point comes from the turn-angle signal of the smoothed boundary:

- A **corner** is *concentrated* turn: at least $12°$ inside a $\pm 1$-sample window that also accounts for at least $55\%$ of the turn in the surrounding $\pm 5$-sample window. This is the fix for Potrace's alpha problem—an impulse test rather than a deviation test, so a sharp 20° serif join (impulse) separates cleanly from a tight smooth arc (distributed turn).
- A **straight run** is a maximal stretch whose chord deviation stays under $\max(0.15\,\mathrm{px},\ 0.005\,\ell)$ for chord length $\ell$. The envelope is linear in $\ell$ on purpose: a true line's deviation is flat noise at any length, while an arc of radius $r$ deviates from its chord by the sagitta $\ell^2/8r$—quadratic in $\ell$—so a linear envelope separates lines from arcs at every scale with one pair of constants.
...
**5. Continuity passes.** Corners that rasterization rounded into tiny curves between straight edges are rebuilt as sharp intersections. Smooth joins get G1 alignment, then the G2 "harmonization" rule used by the Glyphs and FontLab editors. For cubics $a_0 a_1 a_2 a_3$ and $b_0 b_1 b_2 b_3$ meeting at $a_3 = b_0$, intersect the handle lines $a_1 a_2$ and $b_1 b_2$ at $d$ and form the ratios

$$
p_0 = \frac{|a_1 a_2|}{|a_2 d|}, \qquad p_1 = \frac{|d\, b_1|}{|b_1 b_2|}, \qquad p = \sqrt{p_0\, p_1},
$$

then move the join point to the fraction $t = p/(p+1)$ of the way along $a_2 \to b_1$. This equalizes curvature across the join.

**6. Raster-loss refinement.** This is where the diffvg idea pays off, in a form cheap enough to run in a CLI tool. The prefilter approximation says that near the outline, a pixel's ink coverage is approximately a linear ramp in its signed distance to the path:

$$
c(x) \;\approx\; \operatorname{clamp}\!\left(\tfrac{1}{2} + s(x),\; 0,\; 1\right),
$$

with $s(x)$ in pixels, signed toward the ink. A candidate outline $\Gamma$ is then scored by its mean squared coverage error over the band $B$ of pixel centers within 2px of the original boundary:

$$
L(\Gamma) \;=\; \frac{1}{|B|} \sum_{x \in B} \bigl(c_\Gamma(x) - c_{\mathrm{src}}(x)\bigr)^2.
$$

This is a smooth, honest score for *any candidate outline*—including candidates with different structure than the current one, which is exactly what makes it usable as a judge. And because stages 1–5 already produce a sub-pixel-accurate outline, there's no need for autodiff or gradient descent: the few remaining degrees of freedom per decision can be solved by golden-section search.

One amusing wrinkle: optimizing a cubic's two handle lengths $(\alpha, \beta)$ independently stalls on arcs, because the loss forms a narrow diagonal valley along $\alpha + \beta \approx \mathrm{const}$—many cubics trade tension between their handles almost freely. Searching the mean direction $\tfrac{1}{2}(\alpha + \beta)$ and the difference direction $\tfrac{1}{2}(\alpha - \beta)$ before the individual axes fixes it. This is the kind of thing the differentiable-rasterization papers handle with momentum; with two variables you can just look at the valley and aim along it.
